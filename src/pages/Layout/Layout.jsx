...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    document.title = "USV Adeverinte";
    if (pathSegments.length > 0) {
      const segment = pathSegments[0];
      const title = segment.charAt(0).toUpperCase() + segment.slice(1);
      document.title = `USV Adeverinte - ${title}`;
      setLinks(getLinksForRoute(user, segment));
    } else {
      document.title = "USV Adeverinte - Home";
  ...
  return (
    <div className={`${styles.layout} ${isSidebarOpen ? styles.sidebarOpen : styles.sidebarClosed}`}>
      {links && links.length > 0 && (
        <PageTitle title={document.title.replace('USV Adeverinte - ', '')} secondNav={links} />
      )}
